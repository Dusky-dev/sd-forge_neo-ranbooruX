...
class ControlNetForForgeOfficial(scripts.Script):
    ...
    def get_input_data(self, p, unit, preprocessor, h, w):
        ...
        if unit.input_mode == external_code.InputMode.MERGE:
            # ... [rest of the MERGE block remains unchanged] ...
            ...
            # ... [rest of the BATCH block remains unchanged] ...
            image_list = []
            ...
            # Check if unit.image is the dictionary structure
            is_dict_input = isinstance(unit.image, dict) and "image" in unit.image

            # Extract image data correctly
            unit_image_data = unit.image["image"] if is_dict_input else unit.image
            unit_image_fg = unit.image_fg[:, :, 3] if unit.image_fg is not None else None # Assuming image_fg remains separate

            if unit.use_preview_as_input and unit.generated_image is not None:
                image = unit.generated_image
            elif unit_image_data is None: # Check the extracted image data
                # Fallback to img2img input if ControlNet input is empty
                if a1111_i2i_image is None:
                     raise ValueError("ControlNet enabled but no input image provided in ControlNet Unit or img2img tab.")
                resize_mode = external_code.resize_mode_from_value(p.resize_mode)
                image = HWC3(np.asarray(a1111_i2i_image))
                using_a1111_data = True
                logger.info("ControlNet input image not found, using img2img init image instead.")
            elif unit_image_fg is not None and (unit_image_data < 5).all() and (unit_image_fg > 5).any():
                # Use foreground mask if base image is effectively black/empty and fg mask exists
                image = unit_image_fg
                logger.info("ControlNet input image is empty, using foreground mask as input image.")
            else:
                image = unit_image_data # Use the primary image data

            if not isinstance(image, np.ndarray):
                # Handle cases where image might still not be an array (e.g., base64 string from API)
                # This part might need further refinement depending on how non-array data is handled elsewhere
                if isinstance(image, str):
                     try:
                         image = decode_base64_to_image(image) # Example API handling
                     except Exception as e:
                         raise ValueError(f"Could not decode base64 image: {e}")
                else:
                    raise ValueError(f"ControlNet input image is not a valid numpy array. Type: {type(image)}")

            image = HWC3(image) # Ensure HWC3 format

            # Extract mask data correctly
            unit_mask_data = unit.image["mask"] if is_dict_input else unit.mask_image # Check dict first, then separate attribute
            unit_mask_image_fg = unit.mask_image_fg[:, :, 3] if unit.mask_image_fg is not None else None

            if using_a1111_data:
                # Use img2img mask if we fell back to img2img image
                ...
                mask = unit_mask_image_fg # Prioritize separate foreground mask input if it exists
            elif unit_mask_data is not None and (unit_mask_data > 5).any():
                mask = unit_mask_data # Use mask from dict or separate attribute
            elif unit_image_fg is not None and (unit_image_fg > 5).any():
                # Fallback to using the foreground image alpha channel as mask if no other mask is present
                mask = unit_image_fg
            else:
                mask = None # No mask found
        ...
        if resize_mode == external_code.ResizeMode.OUTER_FIT and preprocessor.expand_mask_when_resize_and_fill:
            # ...
            pass # This part remains the same
...
def process(self, p, *args, **kwargs):
    # --- Main setup method, called once per generation ---
    logger.info("ControlNet process START") # <<< ADD
    self.current_params = {} # Reset params for this run
    logger.info("ControlNet process: self.current_params initialized.") # <<< ADD

    enabled_units = [] # <<< ADD initialization
    try:
        # Reconstruct units from the flat *args tuple
        enabled_units = self.get_enabled_units(args)
        logger.info(f"ControlNet process: Found {len(enabled_units)} enabled units.") # <<< ADD
    except Exception as e:
         logger.error(f"ControlNet: Failed to parse script arguments in process(): {e}", exc_info=True) # <<< MODIFY log context
         return # Stop processing if args can't be parsed

    if not enabled_units:
         logger.info("ControlNet process: No units enabled.") # <<< MODIFY log context
         return

    # Write infotext if units are enabled
    try: # <<< ADD try/except
        Infotext.write_infotext(enabled_units, p)
    except Exception as e:
        logger.error(f"ControlNet process: Error writing infotext: {e}", exc_info=True) # <<< ADD

    # Process each enabled unit
    for unit in enabled_units:
         # unit_idx = unit.cn_idx # Get original index stored during reconstruction # cn_idx might not exist if get_enabled_units failed partially
         unit_idx = getattr(unit, 'cn_idx', -1) # Safely get index, default to -1
         logger.info(f"ControlNet process: Starting Unit {unit_idx} (Module={unit.module}, Model={unit.model})") # <<< MODIFY log context
         params = None # <<< Initialize params here
         try:
             self.bound_check_params(unit) # Check parameters first
             params = ControlNetCachedParameters()
             # Run the main setup logic for this unit
             logger.info(f"ControlNet process: Calling process_unit_after_click_generate for Unit {unit_idx}") # <<< ADD
             self.process_unit_after_click_generate(p, unit, params, *args, **kwargs)
             # Store the successfully processed parameters
             self.current_params[unit_idx] = params
             logger.info(f"ControlNet process: Successfully processed Unit {unit_idx}. Stored params.") # <<< ADD
         except Exception as e:
             logger.error(f"ControlNet process: Error processing Unit {unit_idx}: {e}", exc_info=True) # <<< MODIFY log context
             self.current_params[unit_idx] = None # Mark unit as failed

    logger.info(f"ControlNet process END. Final state: {list(self.current_params.keys())}") # <<< MODIFY log context
